...
export class AuthService {
  ...
  async signIn(signInDto: SignInDto) {
    try {
      console.log('object: in the signin');
      ...
      console.log(`Tokens:  , ${accessToken}, ${refreshToken}, User:  ${user}`);

      const adminRole = [2001, 5150];
      const cfOperatorRole = [2001, 1984];
      return {
        user: user.name,
        roleName: user.role,
        roles: user.role === 'admin' ? adminRole : cfOperatorRole,
        // tokenType: 'Bearer',
        accessToken,
        refreshToken,
      };
